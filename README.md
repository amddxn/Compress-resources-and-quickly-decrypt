...
主界面的处理进度卡片会实时标记“准备任务、修改后缀、解压与检查、处理完成”四个阶段。修改阶段按实际文件数更新；解压时会实时读取 Bandizip 输出的当前压缩包真实百分比，同时显示当前层已完成任务数，并在扫描下一层、等待密码或等待嵌套文件确认时切换对应状态。如果当前 Bandizip 版本或压缩格式不输出百分比，进度条会明确切换为不确定动画，不会用任务数伪装成解压百分比。
